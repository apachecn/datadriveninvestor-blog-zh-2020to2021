...
> 原文：<https://medium.datadriveninvestor.com/advanced-python-made-easy-part-2-b741b76b2411?source=collection_archive---------1----------------------->

## 使用这些技巧和技术…

![](img/5204160a8fec7f13987aad5d1720a3f9.png)

Pic credits : Unsplash.com ([Markus Spiske](https://unsplash.com/@markusspiske))

这是本系列的第 2 部分。你可以阅读下面的第一部分:
...
在这一部分中，我们将看到一些高级的 python 构造，它们将帮助您编写高效的代码，提高代码的可读性和性能。
...
Lambda 用于使用*“lambda”*关键字创建小型匿名函数，可以在任何需要函数对象的地方使用。它可以有任意数量的参数，但只能有一个表达式。
...
```
def cal(n):
    return lambda t : t ** nvar = cal(3)print(var(12))
```
...
如果你想给一个函数传递多个参数或关键字参数，那么使用`***args**`和`****kwargs**` **。**
...
Python 3.0 增加了函数注释，这在类型检查时很有帮助，因为 Python 支持动态类型。它们收集关于使用的参数类型和函数返回类型的信息。它们可以和单个参数、函数参数、嵌套函数、装饰符等一起使用。
...
*OrderedDict* 是高性能容器数据类型之一，是 Dict 对象的子类。它保持键的插入顺序。在删除或重新插入密钥的情况下，顺序保持不变。
...
dict2['a'] = 'A'
dict2['b'] = 'B'
dict2['c'] = 'C'for key, value in dict2.items():
    print (key, value)
```
